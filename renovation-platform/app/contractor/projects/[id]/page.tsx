...
// New imports
import ProjectSubcontractors from "./subcontractors"
...
const documentTypes = {
  blueprints: {
    title: "Blueprints & Design Documents",
    description: "Floor plans, elevations, 3D renderings, and design specifications (provided by homeowner)",
    required: false,
    autoGenerated: false,
...
export default function ProjectDetailsPage({ params }: { params: { id: string } }) {
  ...
  const handleMilestoneUpdate = (id: string, status: string) => {
    setProject((prev) => ({
      ...prev,
      milestones: prev.milestones.map((milestone) => (milestone.id === id ? { ...milestone, status } : milestone)),
      progress: calculateProgress([
        ...prev.milestones.map((milestone) => (milestone.id === id ? { ...milestone, status } : milestone)),
      ]),
    }))

    toast({
      title: "Milestone Updated",
      description: `Milestone status has been updated to ${status}.`,
    })
  }

  // Calculate progress based on milestones
  const calculateProgress = (milestones: typeof project.milestones) => {
  ...
  const handlePaymentUpdate = (id: string, status: string) => {
    setProject((prev) => ({
      ...prev,
      payments: prev.payments.map((payment) => (payment.id === id ? { ...payment, status } : payment)),
    }))
          ...
          <Dialog open={isStatusDialogOpen} onOpenChange={setIsStatusDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>Change the current status of this project.</DialogDescription>
              </DialogHeader>
          ...
          <Dialog open={isMessageDialogOpen} onOpenChange={setIsMessageDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>Send a message to {project.client.name} regarding this project.</DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                <div className="grid gap-2">
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          {payment.status === "not-paid" && (
                            <Button
                              variant="ghost"
                              size="icon"
                              onClick={() => handlePaymentUpdate(payment.id, "pending")}
                      ...
                      <Select value={selectedDocType || ""} onValueChange={setSelectedDocType}>
                        ...
                        <SelectContent>
                          {Object.entries(documentTypes).map(([key, value]) => (
                            <SelectItem key={key} value={key} disabled={value.autoGenerated}>
                              {value.title} {value.autoGenerated && "(Auto-generated)"}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
              ...
              {getDocumentCount() === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-sm text-muted-foreground max-w-md mt-1 mb-4">
                    Upload project documents to keep track of important files and share them with your team.
                  </p>
                    ...
                    return (
                      <div key={key}>
                        <h3 className="text-lg font-medium mb-2 flex items-center">
                          ...
                          {value.required && docs.length === 0 && !value.autoGenerated && (
                        ...
                        {docs.length === 0 ? (
                          <div className="text-sm text-muted-foreground">
                            {value.autoGenerated
                              ? "No materials have been added to generate documents from."
